...
def search_web(state: InterviewState):
    ...
    query_text = getattr(search_query, "search_query", None) or "default topic query"
...
def search_wikipedia(state: InterviewState):
    ...
    # Validate LLM output
    query_text = getattr(search_query, "search_query", None)
    if not query_text:
        query_text = "default topic query"  # fallback query

    # Retrieve Wikipedia documents
    try:
        search_docs = WikipediaLoader(query=query_text, load_max_docs=2).load()
    except Exception as e:
        # fallback empty list in case of WikipediaLoader failure
        search_docs = []
...
def initiate_all_interviews(state: ResearchGraphState):
    ...
    # Previously this checked human feedback; we now always initiate interviews automatically
    topic = state["topic"]
...
# human_feedback removed from graph wiring (we keep function definition above in case you want it later)
...
# Instead of going to a human_feedback node, we now always initiate interviews automatically
